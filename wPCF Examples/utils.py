...
def returnPCFContribution(point_ind, pointDensity, PCF_radii, d, x0, y0, domainX, domainY):
    ...
    assert ((totalAreas * pointDensity).all() > 0)
...
def getAnnulusAreasAroundPoints(points_i, dr, maxR, l):
    ...
    for annulus in range(len(PCF_radii_lower)):
        ...
        areas_in = vfunc_returnAreaOfCircleInDomainAroundPoint(index=np.arange(len(points_i)), points=points_i, r=inner, domainX=l, domainY=l)
        areas_out = vfunc_returnAreaOfCircleInDomainAroundPoint(index=np.arange(len(points_i)), points=points_i, r=outer, domainX=l, domainY=l)
